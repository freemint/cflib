...
/* Farbicon-Routinen. Basieren auf denen der MyDials von Olaf Meisiek */
...
static void fill_farbtbl(_WORD screen_planes, _WORD handle)
{
	if ((screen_planes > 8) && !pal_init)
	{
		...
		static _WORD const pixtbl[16] = { 0, 2, 3, 6, 4, 7, 5, 8, 9, 10, 11, 14, 12, 15, 13, 16 };

		for (color = 0; color < 255; color++)
		{
			if (color < 16)
			{
				vq_color(handle, pixtbl[color], 1, rgb_palette[color]);
				rgb_palette[color][3] = pixtbl[color];
			}
			else
			{
				vq_color(handle, color - 1, 1, rgb_palette[color]);
...
static void fix_cdata(short *col_data, long len, int old_planes, int new_planes)
{
	long x, i, old_len, rest_len, mul[32], pos;
	short np, *new_data, mask, pixel, bit, color, back[32], old_col[32], maxcol = 0;
	...
	else			/* TrueColor, bzw RGB-orientierte Pixelwerte */
	{
		if (old_planes < 8)
		{
			...
			memcpy(old_col, farbtbl[maxcol], new_planes * sizeof (short));
			memset(farbtbl[maxcol], 0, new_planes * sizeof (short));
		}
		...
		if (old_planes < 8)
			memcpy(farbtbl[maxcol], old_col, new_planes * sizeof (short));
	}
}

CICON *fix_cicon(CICONBLK *cicnblk, _WORD screen_planes, _WORD handle)
{
	_WORD best_planes, find_planes;
