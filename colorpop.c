...
static _WORD fattrib[5], lattrib[6];	/* Sicherung fÅr FÅll- und Linienattribute */
 ...
 */
static void save_and_init_attribs(void)
{
 ...
 */
static void restore_attribs(void)
{
...
static void draw_non_3d(PARMBLK *parmblk)
{
	_WORD pxy[12];
	_WORD color = (_WORD) parmblk->pb_parm;
	_WORD state = parmblk->pb_currstate;
...
static void draw_3d(PARMBLK *parmblk)
{
	_WORD pxy[12];
	_WORD color = (_WORD) parmblk->pb_parm;
	...
	if (color == T_NOCOLOR)
	{
		_WORD movewidth = flags & F_USEROBJ ? 1 : 2;
 ...
 * darstellt
 */
static _WORD __CDECL colorpop_userdef(PARMBLK *parmblk)
{
	_WORD pxy[4];
	long flags = parmblk->pb_parm;
 ...
 */
static _WORD is_colorpop_object(OBJECT *tree, _WORD obj)
{
	return (tree[obj].ob_type & 0xff) == G_USERDEF
		&& ((tree[obj].ob_spec.userblk == &userblk)
			...
			&& tree[obj].ob_spec.userblk < popobj_userblk + MAX_COLORPOP));
}
...
{
	assert(is_colorpop_object(tree, obj));
	return (_WORD) (tree[obj].ob_spec.userblk->ub_parm & 0xffff);
}
...
_WORD init_colorpop(_WORD maxplanes)
{
	...
	if (poptree)
		poptree = (OBJECT *) realloc(poptree, (maxcolors + 2) * sizeof(OBJECT));
	else
		poptree = (OBJECT *) calloc(maxcolors + 2, sizeof(OBJECT));
 ...
 */
static void ini_colorpop(_WORD planes, _WORD show_noncolor)
{
	_WORD colors;
	_WORD i;
	_WORD x = 0, y = 0;
	...
	poptree[0].ob_height = startframe + colors * size / maxx + (show_noncolor ? size : 0);
...
/* Farbpopup ausfÅhren
 */
_WORD do_colorpop(_WORD x, _WORD y, _WORD act_color, _WORD planes, _WORD show_noncolor)
{
	MENU menu, cmenu;
	GRECT r;
