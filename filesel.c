...
/* -- MagiC ------------------------------------------------------------------ */
static int do_magx(char *path, char *name, char *mask, char *title, FSEL_CB open_cb)
{
	int ok = FALSE;
	void *fsl;
	char def_mask[] = "*\0\0";
	char pat[50];
	char *p;
	char hist_path[256] = "\0\0";
	_WORD num, flags = 0, but, i, j, win, ret;
	EVNT ev;
	...
	{
		i = SORTBYNAME;
		fsl = fslx_do(title, path, 256, name, 60, pat, 0L, hist_path, &i, flags, &but, &num, &p);
		if (fsl)
		{
		}
	}
	...
	{
		fsl = fslx_open(title, -1, -1, &win, path, 256, name, 60, pat, 0L, hist_path, SORTBYNAME, flags);
		if (fsl)
		{
			disable_menu();
			do
			{
				...
				if (ev.mwhich & MU_MESAG)
				{
					switch (ev.msg[0])
					{
						...
						case WM_MOVED:
						case WM_SIZED:
							if (ev.msg[3] != win)	/* fr fremdes Fenster */
							{
				...
				ret = fslx_evnt(fsl, &ev, path, name, &but, &num, &i, &p);
			} while (ret == 1);
			enable_menu();
		}
	}
	...
	if (slct_check())
	{
		if (open_cb != NULL)		/* Selectric-mehrfach Auswahl */
		{
			for (i = 0; i < SLCT_ANZ; i++)
				files[i] = (char *) cf_malloc(256, "select_file", FALSE);
			slct->comm = 1;
